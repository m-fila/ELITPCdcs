...
#include <iostream>
abstract_variable::abstract_variable(std::string ObjectName, std::string VariableName)
    : ObjectName(ObjectName), VariableName(VariableName),
      FullName(ObjectName + "." + VariableName) {
...
void abstract_variable::handler_ValueChanged(UA_Client *client, UA_UInt32 subId,
    ...
    Database->logData(variable->FullName, std::to_string(value->sourceTimestamp) + "," +
                                              variable->translateValue());
}

void abstract_variable::addMonitoredItem(UA_Client *client,
                                         UA_CreateSubscriptionResponse response) {
    UA_MonitoredItemCreateRequest monRequest =
        UA_MonitoredItemCreateRequest_default(VariableNodeId);
