...
void opc_client::stateCallback(UA_Client *client, UA_SecureChannelState channelState,
    ...
    if(channelState == UA_SECURECHANNELSTATE_OPEN &&
       context->connection != Connection::Opened) {
        ...
        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,
                    "A connection with the server is closed");
...
void opc_client::addSubscription(UA_Double interval) {
    ...
    UA_CreateSubscriptionResponse response =
        UA_Client_Subscriptions_create(client, request, nullptr, nullptr, nullptr);
