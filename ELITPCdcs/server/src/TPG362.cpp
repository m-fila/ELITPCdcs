...
std::string TPG362::setGaugesStatus(STATUS s1, STATUS s2) {
  ...
  return sendWithEnquiry("SEN," + std::to_string(S1) + "," +
                         std::to_string(S2));
}
...
std::string TPG362::setDisplayResolution(int r1, int r2) {
  return sendWithEnquiry("DCD," + std::to_string(r1) + "," +
                         std::to_string(r2));
}

std::string TPG362::setSwitchingFunction(SWITCHING_FUNCTION f,
                                         SWITCHING_STATUS s,
                                         double lowThreshold,
  ...
  ss << function << "," << status << "," << lowThreshold << ","
     << highThreshold;
...
std::string TPG362::setFilter(FILTER f1, FILTER f2) {
  ...
  return sendWithEnquiry("FIL," + std::to_string(F1) + "," +
                         std::to_string(F2));
}
