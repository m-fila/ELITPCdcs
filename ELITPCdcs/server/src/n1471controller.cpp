...
N1471Controller::N1471Controller(std::string name): 
    OpcTemplateController<UA_DT1415m,UA_DT1415c,N1471>(name,
        UA_TYPES_DCSNODESET[UA_TYPES_DCSNODESET_DT1415M],
        UA_TYPES_DCSNODESET[UA_TYPES_DCSNODESET_DT1415C]){
}

void N1471Controller::init(UA_Server* server){
    addObject(server);
...
UA_DT1415m N1471Controller::getMeasurements(){
    UA_DT1415m dtm;
    UA_DT1415m_init(&dtm);
    int size=4;
...
UA_DT1415c N1471Controller::getSettings(){
    UA_DT1415c dtc;
    UA_DT1415c_init(&dtc);
    int size=4;
...
UA_StatusCode N1471Controller::setChannelCallback(UA_Server *server,
                                         const UA_NodeId *sessionId, void *sessionHandle,
                                         const UA_NodeId *methodId, void *methodContext,
                                         const UA_NodeId *objectId, void *objectContext,
                                         size_t inputSize, const UA_Variant *input,
                                         size_t outputSize, UA_Variant *output){
    N1471Controller* Monitor=static_cast<N1471Controller*>(methodContext);
    if(Monitor->isConnected()){
        ...
        if(state){
           // Monitor->device.setON(CH);
            DeviceCommand<N1471> command=std::bind(&N1471::setON, _1,CH);
            Monitor->buffer.push(command);
        }
        else{
            //Monitor->device.setOFF(CH);
            DeviceCommand<N1471> command=std::bind(&N1471::setOFF, _1,CH);
            Monitor->buffer.push(command);
        }
...
void N1471Controller::addSetChannelMethod(UA_Server* server){
...
UA_StatusCode N1471Controller::setVoltageCallback(UA_Server *server,
                         const UA_NodeId *sessionId, void *sessionHandle,
                         const UA_NodeId *methodId, void *methodContext,
                         const UA_NodeId *objectId, void *objectContext,
                         size_t inputSize, const UA_Variant *input,
 ...
 //   UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_SERVER, "SetOutput was called");
    ...
    if(Monitor->isConnected()){
        ...
        N1471::CHANNEL CH=static_cast <N1471::CHANNEL>(channel);
        DeviceCommand<N1471> command=std::bind(&N1471::setVoltageSet, _1,CH,voltage);
        Monitor->buffer.push(command);
...
void N1471Controller::addSetVoltageMethod(UA_Server *server) {
...
UA_StatusCode N1471Controller::setVoltageMaxCallback(UA_Server *server,
                         const UA_NodeId *sessionId, void *sessionHandle,
                         const UA_NodeId *methodId, void *methodContext,
                         const UA_NodeId *objectId, void *objectContext,
                         size_t inputSize, const UA_Variant *input,
 ...
 //   UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_SERVER, "SetOutput was called");
    ...
    if(Monitor->isConnected()){
        ...
        DeviceCommand<N1471> command=std::bind(&N1471::setVoltageMax, _1,CH,voltage);
        Monitor->buffer.push(command);
    }
    else {
...
void N1471Controller::addSetVoltageMaxMethod(UA_Server *server) {
...
UA_StatusCode N1471Controller::setRampUpCallback(UA_Server *server,
                         const UA_NodeId *sessionId, void *sessionHandle,
                         const UA_NodeId *methodId, void *methodContext,
                         const UA_NodeId *objectId, void *objectContext,
                         size_t inputSize, const UA_Variant *input,
 ...
 //   UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_SERVER, "SetOutput was called");
    ...
    if(Monitor->isConnected()){
       ...
       // Monitor->device.setRampUp(CH,voltage);
        DeviceCommand<N1471> command=std::bind(&N1471::setRampUp, _1,CH,voltage);
        Monitor->buffer.push(command);
    }
    else {
...
void N1471Controller::addSetRampUpMethod(UA_Server *server) {
...
UA_StatusCode N1471Controller::setRampDownCallback(UA_Server *server,
                         const UA_NodeId *sessionId, void *sessionHandle,
                         const UA_NodeId *methodId, void *methodContext,
                         const UA_NodeId *objectId, void *objectContext,
                         size_t inputSize, const UA_Variant *input,
 ...
 //   UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_SERVER, "SetOutput was called");
    ...
    if(Monitor->isConnected()){
        ...
        DeviceCommand<N1471> command=std::bind(&N1471::setRampDown, _1,CH,voltage);
        Monitor->buffer.push(command);
    }
    else {
...
void N1471Controller::addSetRampDownMethod(UA_Server *server) {
...
void N1471Controller::addSetCurrentMethod(UA_Server *server) {
...
UA_StatusCode N1471Controller::setCurrentCallback(UA_Server *server,
                         const UA_NodeId *sessionId, void *sessionHandle,
                         const UA_NodeId *methodId, void *methodContext,
                         const UA_NodeId *objectId, void *objectContext,
                         size_t inputSize, const UA_Variant *input,
 ...
 //   UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_SERVER, "SetOutput was called");
    ...
    if(Monitor->isConnected()){
        ...
        N1471::CHANNEL CH=static_cast <N1471::CHANNEL>(channel);
        DeviceCommand<N1471> command=std::bind(&N1471::setCurrentSet, _1,CH,current);
        Monitor->buffer.push(command);
    }
    else {
