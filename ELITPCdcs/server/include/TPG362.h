...
public:
    ...
    enum class SWITCHING_FUNCTION {F1=1, F2=2,F3=3,F4=4};
    ...
    std::string getUnits();
    std::string getSwitchingFunctions();
    std::string getSwitchingFunction(SWITCHING_FUNCTION f);
    std::string getFilter();

    std::string setFormat(FORMAT format);
    std::string setGaugesStatus(STATUS s1=STATUS::nochange,STATUS s2=STATUS::nochange);
    std::string setUnits(UNIT unit);
    std::string setDisplayResolution(int r1=0,int r2=0);
    std::string setSwitchingFunction(SWITCHING_FUNCTION f, SWITCHING_STATUS s, double lowThreshold, double highThreshold);
