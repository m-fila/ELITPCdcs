...
void MKS946_controller::callSetUnits(int unit) {
...
void MKS946_controller::RelayChangedCallback(UA_Client *client, UA_UInt32 subId,
                                             void *subContext, UA_UInt32 monId,
                                             void *monContext, UA_DataValue *value) {
    if(value->hasValue) {
        if(!UA_Variant_isEmpty(&value->value)) {
            void *data = value->value.data;
            auto *context = static_cast<MKS946_controller *>(monContext);
