...
MKS910Widget::MKS910Widget(std::string name, std::string address,
...
void MKS910Widget::connectSignals() {
  ...
  connect(unitsBox, SIGNAL(currentIndexChanged(int)), this,
...
void MKS910Widget::updateMeasurements(void *data) {
  ...
  MKS910codes::Units units =
      static_cast<MKS910codes::Units>(measurements.units);
  val = QString::fromStdString(MKS910codes::unitsToString.at(units));
  mUnitLabel->setText(val);
  MKS910codes::Status status =
      static_cast<MKS910codes::Status>(measurements.status);
  if (status == MKS910codes::Status::O) {
    val = QString::fromStdString("OK");
  } else {
    val =
        QString::fromStdString(MKS910codes::statusToString.at(status) + "fail");
  }
  mStatus->setText(val);
