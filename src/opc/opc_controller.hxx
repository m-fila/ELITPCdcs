...
void opc_template_controller<T,D>::updateMeasurements(UA_Server *server){
    if(isConnected()){
        ...
        clearTemporaryVariable(now);
        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "Measurements updated");
    }
    else{
        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "Updating measurements failed, physical device disconneted");
    }
}

template <class T,class D>
void opc_template_controller<T,D>::updateSettings(UA_Server *server){
    if(isConnected()){
        T now =getSettings();
        UA_Variant value;
        UA_Variant_setScalar(&value, &now, &VariableType);
        UA_NodeId NodeId = UA_NODEID_STRING_ALLOC(1, SettingsVariableName.c_str());
        UA_Server_writeValue(server, NodeId, value);
        UA_NodeId_deleteMembers(&NodeId);
        clearTemporaryVariable(now);
        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "Settings updated");
    }
    else{
        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "Updating settings failed, physical device disconneted");
    }
}
